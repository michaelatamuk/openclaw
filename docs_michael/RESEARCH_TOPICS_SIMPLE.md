...
## Topic 1: Saving Money on AI Costs by Keeping Prompts Stable
...
last call instead of re-processing everything — similar to a photo copier that only
...
## Topic 2: What Happens When an AI Runs Out of Memory
...
## Topic 3: What to Do When an AI Provider Goes Down
...
## Topic 4: Connecting to 25 Different Messaging Platforms Without Going Mad
...
## Topic 5: How to Stream AI Responses into Messaging Apps
...
## Topic 6: How OpenClaw Remembers Things About You Over Time
...
## Topic 7: Managing AI Agents That Spin Up Other AI Agents
...
## Topic 8: How to Safely Let a Personal AI Listen to Your Messages
...
## Topic 9: Translating Between Different AI Communication Protocols
...
## Topic 10: Keeping 100+ Plugins From Breaking Each Other
...
|---|---|---|---|
| Prompt caching (T1) | Very new | Easy — can measure on real system | Very high — affects every AI app |
| Context compaction (T2) | Fairly new | Medium | High — unsolved across all AI |
| Provider failover (T3) | Moderate | Easy — can simulate failures | Medium |
| Multi-channel abstraction (T4) | Moderate | Hard — broad survey needed | Medium |
| Streaming delivery (T5) | Somewhat familiar | Easy — can run user studies | Medium |
| Hybrid memory (T6) | Moderate | Easy — can benchmark | High — RAG for personal AI |
| Sub-agent orchestration (T7) | Very new | Medium — needs fault injection | Very high — agent reliability |
| Device pairing trust (T8) | Very new | Hard — needs formal analysis | Medium |
| Protocol bridge (T9) | Moderate | Hard — mostly a case study | Medium |
| Plugin boundary enforcement (T10) | Moderate | Medium — can study empirically | Medium |

### Where to Start

**Fastest path to a publishable paper**: Topic 1 (prompt caching) or Topic 5 (streaming
delivery). Both involve concrete engineering decisions with outcomes you can measure on
a real working system. No special lab setup needed.

**Biggest potential impact if done well**: Topic 2 (compaction) or Topic 6 (hybrid
memory). Both tackle problems that affect every AI assistant today, and OpenClaw
provides a production baseline to compare new ideas against.

**Most original — nobody has written this yet**: Topic 7 (sub-agent reliability) or
Topic 8 (personal AI trust model). There is almost no existing literature on either.
OpenClaw gives researchers a concrete system with real code to analyze and build on.
...
|---|---|
| T1: Prompt caching | The `src/agents/` folder, files related to "system-prompt" and "cache-boundary" |
| T2: Context compaction | The `src/agents/` folder, the file called `compaction.ts` |
| T3: Provider failover | The `src/agents/` folder, files related to "failover" and "auth-health" |
| T4: Multi-channel | The `extensions/` folder (all the individual platform adapters) and `src/routing/` |
| T5: Streaming delivery | The `src/agents/` folder, files related to "block-chunker" and "streaming" |
| T6: Memory | `src/agents/memory-search.ts` and the `extensions/active-memory/` folder |
| T7: Sub-agents | The `src/agents/` folder, files related to "subagent-registry" |
| T8: Trust and pairing | The `src/pairing/` folder |
| T9: Protocol bridge | The `src/acp/` folder, especially `translator.ts` |
| T10: Plugin boundaries | The `extensions/` folder and `src/plugin-sdk/` |
